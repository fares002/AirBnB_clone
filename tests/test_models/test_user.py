...
class TestUser_instantiation(unittest.TestCase):
    ...
    def testnoarginstan(self):
        self.assertEqual(User, type(User()))

    def testinstanobjects(self):
        self.assertIn(User(), models.storage.all().values())

    def testidpublicstr(self):
        self.assertEqual(str, type(User().id))

    def testcreated_atpublicdatetime(self):
        self.assertEqual(datetime, type(User().created_at))

    def testupdated_atpublicdatetime(self):
        self.assertEqual(datetime, type(User().updated_at))

    def testemailpublicstr(self):
        self.assertEqual(str, type(User.email))

    def testpasswordpublicstr(self):
    ...
    def testusersids(self):
        user1 = User()
        user2 = User()
        self.assertNotEqual(user1.id, user2.id)

    def testuserscreated_at(self):
        user1 = User()
        sleep(0.05)
        user2 = User()
        self.assertLess(user1.created_at, user2.created_at)

    def testuserupdatedat(self):
        user1 = User()
        sleep(0.05)
        user2 = User()
